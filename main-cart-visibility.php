...
class HS_WC_visibility_Tab {
   ...
   public static function hshidecart_activate_filter() {
     ...
     public static function hshidecart_show_price_logged($price) {
        $pice_option = get_option('wc_settings_tab_visibility_title');
...
function hshidecart_add_custom_field_into_loop() {
    ...
    $show_hide_option = get_post_meta($product_id, 'woo_disable_add_to_cart_checkbox', 'false');
    $start_datetime = get_post_meta($product_id, 'woo_disable_add_to_cart_start_date', 'false');
    $end_datetime = get_post_meta($product_id, 'woo_disable_add_to_cart_end_date', 'false');
    //Start time str 
    ...
    if ($show_hide_option == 'show_button') {
       ...
       if ((strtotime($start_set_time) <= strtotime($current_time)) && ( strtotime($current_time) <= strtotime($end_set_time) )) {
            ...
            ?>
            <style>
                ul.products > li.post-<?php echo $product_id; ?> a.button{display:none !important}
            </style>
            <?php
        }
    }
...
add_action('woocommerce_single_product_summary', 'hshidecart_add_custom_field_into_single', 31,1);

function hshidecart_add_custom_field_into_single($product_id) {
    global $product;
	$product_id = $product->get_id();
	$show_hide_option = get_post_meta($product_id, 'woo_disable_add_to_cart_checkbox', 'false');
    $start_datetime = get_post_meta($product_id, 'woo_disable_add_to_cart_start_date', 'false');
   ...
   if ($show_hide_option == 'show_button') {
         ...
         if ((strtotime($start_set_time) <= strtotime($current_time)) && ( strtotime($current_time) <= strtotime($end_set_time) )) {
            ?>
            <style>
                #product-<?php echo $product_id; ?> div.summary.entry-summary form.cart{display:none !important}
            </style>
            <?php
            }
